...
class SaleOrder(models.Model):
    ...
    def onchange_type_id(self):
        ...
        for order in self:
            ...
            if order_type.picking_type_id:
                vals.update({"picking_type_id": order_type.picking_type_id})
    ...
    def copy_data(self, default=None):
        if default is None:
            default = {}
        if 'type_id' not in default:
            if self.env.user.partner_id.sale_type:
                default['type_id'] = self.env.user.partner_id.sale_type.id
            elif self.type_id:
                default['type_id'] = self.type_id.id
        if 'user_id' not in default:
